...
{
	//Сделать осмотр с учетом комментов начиная от } и до { иск комменты и внося в список
	/*QStringList classes;
	...
	bool isOutExcess = true; // состояния, условия и тп

	QString currentClass;
	register int a;
	for(a=0;a < text.size(); a++)
	{
		if(text[a]=='\\' && text[a+1]=='\*')// Проверка на открытие комментарий
			isComment = true;
		if(text[a]=='\*' && text[a+1]=='\\')// Проверка на закрытие комментария
			isComment = false;
		if (!isComment)
		{
