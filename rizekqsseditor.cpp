	...
	: QMainWindow(parent, flags)
{
	...
	connect(plainTextEdit, SIGNAL(textChanged()), this, SLOT(setPlainTextEditModified())); //是的，如果文本被更改了
...
void RizekQssEditor::rewriteRecentProjects()
{
	...
	for (int a=0;a<6;++a)
	{
		if (settings.value("lastProjects/" + QString::number(a)).toString().isEmpty()) //Ищет пустую строку
		{
			bool isNoCopy = true;
			for (int b=0;b<a;++b)
			{
					if (addressOfCurrenFile==settings.value("lastProjects/" + QString::number(b)).toString()) //Ищет совпадения
					{
						isNoCopy = false;
					}
			}
			if(isNoCopy && isListNotComplete) //Если нет копий то записать
				{
	...
	if (isListNotComplete==false)
	{
		...
		for (int b=0;b<5;++b)
		{
			if (addressOfCurrenFile==settings.value("lastProjects/" + QString::number(b)).toString()) //寻找比赛
			{
				isNoCopy = false;
			}
		}
		if (isNoCopy==true)//如果没有副本
		{
			for(int a=5;a>0;--a)
			{
...
{
	QObject *o = sender(); // 找出发送信号的对象（称为插槽）
	QAction * act = qobject_cast<QAction *>(o); // 尝试将此对象的类型带到预期的 (QAction)
	if (act)
		if(act->text()!=addressOfCurrenFile)
		{
			...
			for(int a=1;a<6;++a)
				if (settings.value("lastProjects/" + QString::number(a)).toString()==act->text()) 
					for(int b=a;b>0;--b) //在返回所需条目之前重置条目
			...
			openFileFromString(act->text()); // 如果强制转换成功，那么我们调用期望对象的所需方法。
		}
}
